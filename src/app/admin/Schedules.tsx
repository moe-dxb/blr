...
export default function SchedulesAdmin() {
  ...
  const save = async () => {
    ...
    try {
      await setDoc(doc(db, 'schedules', selectedUser), { userId: selectedUser, timezone: tz, startTime, endTime, workDays, updatedAt: new Date().toISOString() }, { merge: true });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <Button onClick={save} disabled={!selectedUser}>Save</Button>
      </CardContent>
    </Card>
  );
}